...
def attenuation_factor(Type, elev, h, fx,
    ...
    mask[np.abs(fx)>fcorrmax] = 0
    ix = np.logical_and(np.abs(fx)>fcorrmaxBelieve,np.abs(fx)<= fcorrmax)
...
def calcmoments(fx,vy,fmin=-9999,fmax=9999):
    ...
    moments = {'mm2':mm2,'mm1':mm1,'m0':m0,'m1':m1,'m2':m2,'m3':m3,'m4':m4}
    return moments
...
def compute_SkAs(sf,p,fpfac =None, fbounds = None):
    ...
    if (fpfac == None) and (fbounds == None):
        ...
        pf = band_pass_filter2(sf,p,fmin=fbounds[0],fmax=fbounds[1], retrend=False) 
    elif fbounds == None:
        fx,vy = spectrum_simple(sf,p)
        fp = fx[vy==np.max(vy)]
        if fp==0:
...
def compute_directional_stats_kuijk(fx,theta,S=None, fmin=0.01, fmax = 1.5):
    '''
    following kuijk et al 1988
    
    '''
    # only for range [fmin,fmax]
    dtheta = theta[1]-theta[0]
    sT     = np.sin(theta)
    cT     = np.cos(theta)
    s2T    = np.sin(2*theta)
    ...
    m0     = np.trapz((np.trapz(S[idsel,:], theta)), fx[idsel])

    b1     = np.trapz((np.dot(S[idsel,:], sT))*dtheta,fx[idsel])
    a1     = np.trapz((np.dot(S[idsel,:], cT))*dtheta,fx[idsel])
    b2     = np.trapz((np.dot(S[idsel,:], s2T))*dtheta,fx[idsel])
    a2     = np.trapz((np.dot(S[idsel,:], c2T))*dtheta,fx[idsel])   

    m1     = np.sqrt(a1**2+b1**2)
    theta0rad = np.arctan2(b1, a1)
    theta0 = np.rad2deg(theta0rad) % 360 # cartesian, degrees
    m2     = a2*np.cos(2*theta0rad)+b2*np.sin(2*theta0rad)
    n2     = b2*np.cos(theta0rad)-a2*np.sin(theta0rad)

    dspr2  = 2*(1-np.sqrt((b1/m0)**2+(a1/m0)**2))
    dspr   = np.rad2deg(np.sqrt(dspr2)) # directional spreading in degrees  (sigma)
    skew   = -n2 / ((1-m2)/2)**(3/2)
    kurt   = (6-8*m1+2*m2)/(2*(1-m1))**2 

    return theta0, dspr, skew, kurt

def compute_directional_distribution(fx, theta, S):
    '''
    Directional distribution from 2D spectrum
    '''
    return np.dot(fx, S)/np.trapz(np.dot(fx, S), theta)



def wave_zero_crossing(t, eta, Tmin=0, detrend = True,jaFig = False, returntype='dict'):
    '''
    wave_zero_crossing(sf, eta, detrend=True,jaFig=False)
    ...
    eta : NUMPY ARRAY
        SURFACE ELEVATION [M].
    Tmin : FLOAT
        MINIMUM DURATION OF WAVE TO CONSIDER IN COMPUTING WAVE STATISTICS
    ...
    marlies van der lugt, 12-04-2024 

    '''
    if np.sum(np.isnan(eta))>0:
        print('Signal contains infs or NaNs')
        return {'Hz':np.nan, 'Tz':np.nan, 'Hs':Hs, 'Ts':Ts}
    
    if len(np.atleast_1d(t))==1:
        dt = 1/t
        t = dt * np.arange(0,len(eta))
    ...
    # start with cropping the timeseries to only complete crest-trough series, e.g. find the interval [len3, len4] of indices
    #--------------------------------------------------------------------------
    # detecting the start point of first wave (first upward zero-crossing) 
    len3 = np.nan
    ...
    assert ~np.isnan(len3), 'No upward zero crossing could be identified in the entire timeseries. Perhaps you want to detrend?' 

    # detecting the end point of last wave (last upward zero-crossing )
    len4 = np.nan
    ...
    xUpCross = []; positionXUpCross=[]
    xDownCross = []; positionXDownCross=[]
    # for i in np.arange(len3,len4): 
    #     if eta[i]<=0 and eta[i+1]>=0:
    #         xUpCross.append( 
    #             t[i]-dt/(eta[i+1]-eta[i])*eta[i] )
    ...
    positionXUpCross = np.array(positionXUpCross)
    positionXDownCross = np.array(positionXDownCross)

    #--------------------------------------------------------------------------
    # detecting crest and trough from original data
    ymax = []; ymin = []; xmax = []; xmin = []
    for istart, istop in zip(positionXUpCross[:-1], positionXUpCross[1:]):
        ymax.append( np.max(eta[istart:istop]) )
        ymin.append( np.min(eta[istart:istop]) )

        xmax.append( t[istart+np.argmax(eta[istart:istop])] )
        xmin.append( t[istart+np.argmin(eta[istart:istop])] )
    ...
    # See Abreu 2010 for reference to Stokes for skewness coefficient R 
    R = ymax/(ymax-ymin)
    ...
    # Van der A et al. 2012 (Coastal Eng.) for definition of periods:
    Tc = (xDownCross-xUpCross[:-1])
    Tt = (xUpCross[1:] - xDownCross)
    Tcu = (xmax-xUpCross[:-1])
    Ttu = (xmin-xDownCross)

    # See Abreu 2010 for reference to alpha (Suntoyo et al. 2008)
    alpha = 2*Tc/T

    #--------------------------------------------------------------------------
    #calculating time serie averaged wave parameters
    ix = np.where(T>=Tmin)
    if jaFig:
        print('{:.0f}\% of waves longer than {} s'.format(np.sum(T>=Tmin)/len(T)*100, Tmin))
    ...
    HsortIndex = np.argsort(H2)
    Hsort = H2[HsortIndex]
    HTop3rd = np.round(2/3*len(Hsort))
    Hs= np.mean(Hsort[int(HTop3rd):])
    Tsort = T2[HsortIndex]
    Rsort = R2[HsortIndex]
    alphasort = alpha2[HsortIndex]

    Ts = np.mean(Tsort[int(HTop3rd):])
    Rs = np.mean(Rsort[int(HTop3rd):])
    alphas = np.mean(alphasort[int(HTop3rd):])

    Hz=np.mean(H2) # Zero-crossing mean wave height
    Tz=np.mean(T2) # Zero-crossing mean wave period
    Rz = np.mean(R2)
    alphaz = np.mean(alpha2)

    if jaFig:
        import matplotlib.pyplot as plt
        plt.figure()
        plt.plot(t, eta)
        plt.plot(xUpCross, np.zeros(xUpCross.shape), linestyle='None', marker='o')
        plt.plot(xDownCross, np.zeros(xDownCross.shape), linestyle='None', marker='o')
        plt.plot(xmin, ymin, linestyle='None', marker='o')
        plt.plot(xmax, ymax, linestyle='None', marker='o')
        
    if returntype == 'list':
        return Hz, Tz, alphaz, Rz, Hs, Ts, alphas, Rs
    
    else:
        return {'Hz':Hz, 'Tz':Tz, 'Hs':Hs, 'Ts':Ts, 'alphaz': alphaz, 'alphas': alphas, 'Rz': Rz, 'Rs': Rs, 
                'xup': xUpCross, 'iup': positionXUpCross, 'xdown': xDownCross, 'idown': positionXDownCross, 'R':R, 'alpha': alpha, 'T': T, 'ymax': ymax, 'ymin': ymin}


def compute_rep_wave(t, eta, vars, ix, ppwl = 40, detrend=False, jaPlot = False):

    if detrend:
        eta0 = signal.detrend(eta)
    else:
        eta0 = eta

    tn = np.arange(ppwl+1)/ppwl  # ensure endpoints are included
    etan = []
    ...
    count=0
    for i, (istart, xstart, istop, xstop) in enumerate(zip(vars['iup'][:-1], vars['xup'][:-1], vars['iup'][1:], vars['xup'][1:])):

        # remove waves that are not part of boolean ix:
        if ix[i]==False:
            continue
        else:
            count+=1
        ...
        if jaPlot:
            # add the dimensional wave to the overview plot
            plt.plot(t[istart+1:istop]-t[istart], eta0[istart+1:istop])

        # normalize the waves
        # nt =  (t[istart:istop]-t[istart])/(t[istop]-t[istart])
        nt =  (t[istart+1:istop]-xstart)/(xstop-xstart)
        nt = np.append(np.insert(nt, 0, 0), 1)

        nh = eta0[istart+1:istop]/ (np.max(eta0[istart+1:istop])-np.min(eta0[istart+1:istop])) #istart+1 because istart actually lies in the previous wave
        nh = np.append(np.insert(nh, 0, 0), 0)

        # coarsen the input in case there is more than factor two difference in resolution
        def moving_average(x, w):
            return np.convolve(x, np.ones(w), 'valid') / w
        
        d_res = np.floor(len(nt)/len(tn))
        if d_res>=2:
            nh_coarsen = moving_average(nh, int(d_res)) 
            nh_coarsen = np.append(np.insert(nh_coarsen, 0, 0), 0)
            nt_coarsen = moving_average(nt, int(d_res))
            nt_coarsen = np.append(np.insert(nt_coarsen, 0, 0), 1)
            a=1
            # plt.figure()
            # plt.plot(nt, nh)
            # plt.plot(nt, nh_coarsen)
            # plt.show()
        else:
            nh_coarsen = nh
            nt_coarsen = nt
        ...
        etan.append( np.interp(tn, nt_coarsen, nh_coarsen))


    # compute and plot a representative wave in normalized coordinates
    if len(etan)==0:
        print('no wave meet the criteria')
        return {'t': tn, 'eta':np.nan*np.zeros(tn.shape), 'alpha': np.nan, 'R': np.nan}

    metan = np.mean(np.vstack(etan), axis=0)

    # find Tcn for average wave metan
    Tcn = -999
    for ix in range(ppwl-1):
        if metan[ix]>=0 and metan[ix+1]<=0:
            Tcn = tn[ix]-(1/ppwl)/(metan[ix+1]-metan[ix])*metan[ix] 

    alpha=2*Tcn


    # and the stokes skewness
    h1 = np.max(metan)
    h2 = np.min(metan)
    R = h1/(h1-h2)
    ...
    if jaPlot:
        plt.figure()
        for etani in etan:
            plt.plot(tn, etani, linewidth=0.5)
        plt.plot(tn, metan, linewidth=4, color='k')  
        plt.title('N = {}, alpha={:.2f}, R = {:.2f}'.format(count, alpha, R)) 
        plt.axhline(0, color='k', linestyle='-', linewidth=0.5)
        plt.axhline(0.5, color='k', linestyle='-', linewidth=0.5)
        plt.axhline(-0.5, color='k', linestyle='-', linewidth=0.5)
        plt.axvline(0.5, color='k', linestyle='-', linewidth=0.5)
        plt.axvline(0.25, color='k', linestyle='-', linewidth=0.5)
        plt.axvline(0.75, color='k', linestyle='-', linewidth=0.5)

        plt.xlim([0, 1])
        plt.ylabel('a/H [-]')
        plt.xlabel('t/T [-]')
    
    return {'t': tn, 'eta':metan, 'alpha': alpha, 'R': R}
